...
Этот скрипт создает базу данных SQLite и все необходимые таблицы
...
def main():
    ...
    try:
        ...
        if args.info:
            ...
            print(f"  Путь к файлу: {info['database_path']}")
            print(f"  URL: {info['database_url']}")
            print(f"  Файл существует: {info['file_exists']}")
        ...
        # Показ информации о файле базы данных
        db_path = get_database_path()
        if os.path.exists(db_path):
            size = os.path.getsize(db_path)
            logger.info(f"✅ База данных создана: {db_path} ({size} байт)")
        else:
